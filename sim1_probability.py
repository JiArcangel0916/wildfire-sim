...
PROBABILITIES = [round(p, 1) for p in np.arange(0.1, 1.0, 0.1)]  # 0.1 to 0.9
FIXED_WIND = 'N'   # <-- change this if you want a different fixed wind direction
...
# ── Run simulations ────────────────────────────────────────────────────────────
print(f"=== Simulation 1: Fixed Wind = {FIXED_WIND}, Varying Probability ===")
means, stds = [], []

for prob in PROBABILITIES:
    trial_vals = []
    for t in range(TRIALS):
        print(f"  prob={prob}  trial {t+1}/{TRIALS}", end="\r")
        trial_vals.append(run_simulation(FIXED_WIND, prob, seed=t * 7))
    means.append(np.mean(trial_vals))
    stds.append(np.std(trial_vals))
    print(f"  prob={prob}  → {means[-1]:.1f}% ± {stds[-1]:.1f}%        ")
...
colors = plt.cm.YlOrRd(x)

bars = ax.bar(x, means, width=0.07, color=colors, edgecolor='#555', linewidth=0.8, zorder=3)
...
ax.set_xticks(x)
ax.set_xticklabels([str(p) for p in PROBABILITIES])
ax.set_ylim(0, 100)
ax.set_xlabel('Spread Probability', fontsize=12, color='white')
ax.set_ylabel('% of Forest Burned', fontsize=12, color='white')
ax.set_title(f'Effect of Spread Probability on Forest Burned\n(Wind Direction fixed = {FIXED_WIND})',
...
plt.tight_layout()
plt.savefig('sim1_probability.png', dpi=150, bbox_inches='tight', facecolor=fig.get_facecolor())
print("\nDone! Chart saved as 'sim1_probability.png'")
plt.show()
